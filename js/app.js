(function () {
  ...
  async function supaApi(path, method, body) {
    ...
    if (path === '/api/admin/config' && method === 'PUT') {
      ...
      var prefer = H({ 'Prefer': 'return=representation' });
      if (!BE.hasRow) {
        r = await fetch(BE.url + '/rest/v1/portal_config', { method: 'POST', headers: prefer, body: JSON.stringify({ id: 1, data: data }) });
      } else {
        r = await fetch(BE.url + '/rest/v1/portal_config?id=eq.1', { method: 'PATCH', headers: prefer, body: JSON.stringify({ data: data }) });
        if (r.ok) {
          j = await jread(r);
          if (!Array.isArray(j) || j.length === 0) {   // row abhi nahi bani -> insert
            r = await fetch(BE.url + '/rest/v1/portal_config', { method: 'POST', headers: prefer, body: JSON.stringify({ id: 1, data: data }) });
            j = await jread(r);
          }
        }
        if (r.ok) {
          ...
          if (row) { BE.updatedAt = row.updated_at; BE.hasRow = true; return { ok: true, config: row.data, savedAt: row.updated_at }; }
        }
      }
      if (!r.ok) { var ej = await jread(r); throw err(pubErr(r.status, ej), r.status); }
      var aj = await jread(r); var arow = Array.isArray(aj) ? aj[0] : aj;
      if (arow) { BE.updatedAt = arow.updated_at; BE.hasRow = true; return { ok: true, config: arow.data, savedAt: arow.updated_at }; }
